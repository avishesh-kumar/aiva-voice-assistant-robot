...
class TCPServer:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            logger.info(f"TCPServer listening on {self.host}:{self.port}")

            
        except socket.error as e:
            self._cleanup_server_socket()
            raise OSError(f"Failed to start server on {self.host}:{self.port}: {str(e)}")
    ...
    def accept(self, timeout: Optional[float] = None) -> Tuple[str, int]:
        ...
        try:
            ...
            logger.warning("TCPServer accept timed out")
            raise TimeoutError(f"No client connection within {timeout or self._accept_timeout}s")
        except socket.error as e:
            logger.exception("Failed to accept client connection")
    ...
    def receive(self, max_bytes: int = 4096) -> Optional[bytes]:
        ...
        try:
            ...
            logger.exception("Receive failed, client disconnected")
            self._cleanup_client_socket()
